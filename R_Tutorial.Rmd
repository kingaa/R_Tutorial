...
author: Aaron A. King and others
output:
  html_document:
    toc: yes
    toc_depth: 3
bibliography: R_Tutorial.bib
csl: ecology.csl
---
...
- a programming language called \Slang, developed by John Chambers at Bell Labs, that implements the idea of *programming with data* [@Chambers1998],
...
**In the following, we've endeavored to mark Windows-specific items with a &#9420;&nbsp;and *nix (linux/unix/MacOSX)-specific items with a &#9421;.**
...
**R** is case sensitive:
`Abc` and \textsf{abc} are different variables.
...
?foo
```
(where `foo` is the name of the function you are are seeking help about) into the console window (e.g., try `?sin`). 

- &#9420;&nbsp;The `Help` menu on the tool bar provides links to other documentation, including the manuals and FAQs, and a search facility (`Apropos` on the menu) which is useful if you half remember part of the the name of what it is you need help on.
...
`log10(x)`   | base-10 logarithm, $\log_10{x}$
...
Finally, the summary reports $R^2$ values and the results of an $F$-test comparing the model to the a simple normal null hypothesis.
...
A few of the functions and packages in **R** for statistical modeling and data analysis. 
...
Similarly, commands like `x-5, 2*x, x/10`, and `x^2` apply subtraction, multiplication, and division to each element of the vector. 
...
The first form generates a vector `(from,from+by,from+2*by,...)` with the last entry not extending further than than `to`;
...
http://kinglab.eeb.lsa.umich.edu/R_Tutorial/hurricanes.csv.
...
In the above `xxx` stands for the abbreviated name of the specific distribution (the "**R** name" as given in \cref{tab:distribs}).
...
2. Fit several statistical models to the data and compare them.
1. Graph the results, and save the graph to disk for including in your paper. 
...
- To let **R** know where data and script files are located, you have a few choices:
  - change your working directory to wherever the file(s) are located before running **R**.
  - change your working directory within an **R** session to wherever the file(s) are located using the `setwd()` (**set** **w**orking **d**irectory) function, e.g. `setwd("c:/temp")`.
  - spell out the path to the file explicitly. 
    Use a single forward slash to separate folders (e.g., &#9420;&nbsp;`"c:/My~Documents/R/script.R"` or 
ix&nbsp;`"/home/kingaa/projects/hurricanes"`): this works on all platforms.
