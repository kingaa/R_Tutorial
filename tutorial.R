...
#' The source code for this document is [here](http://github.com/kingaa/R_Tutorial).
...
#' a partial list is in the "contributed documentation" section on the R web site (http://cran.r-project.org/other-docs.html). 
#' This particular version is limited (it has similar coverage to the *Introduction to **R** * that comes with **R** ) and targets biologists who are neither (yet) programmers nor statisticians.
#' 
#' Throughout, we've endeavored to mark Windows-specific items with a &#9420;&nbsp;and *nix (linux/unix/MacOSX)-specific items with a &#9421;.
#' 
#' ## What is **R** ?
...
#' 
#' **R** is an [open source project](https://en.wikipedia.org/wiki/Open-source_software), available for [free download](https://www.r-project.org/) [@R].
...
#' ### Starting **R** 
#' 
#' &#9421;&nbsp;Execute `R` from the command line.
#' 
...
#' Good practice is to make variable names long enough to be evocative, short enough to type.
...
#' #### Exercise
#' 
#' Using editing shortcuts wherever you can, use **R** to compute.
#' 
#' 1. $\frac{2^7}{2^7 - 1}$
#' 1. $({1 - \frac{1}{2^7}})^{-1}$
#' 1. $1+0.2$
...
#' ?foo
#' ```
#' (where `foo` is the name of the function you are seeking help about) into the console window (e.g., try `?sin`). 
#' 
...
#' 
#' ## A first interactive session: descriptive statistics
#' 
...
#' 
#' Table: Some functions for descriptive statistics in **R**
#' 
...
#' In addition, a $t$-test is performed on the hypothesis that each coefficient differs from zero: the $t$-statistic and the corresponding $p$-values are reported.
...
#' The book *Modern Applied Statistics with S* [@Venables2002] gives a good practical overview, and a list of available packages and their contents can be found at the main **R** website (http://cran.r-project.org, and click on `Packages`). 
#' 
...
#' `princomp`, `manova`, `lda`, `cancor`  | Multivariate analysis (also see packages **vegan**, **ade4**)
...
#' **tree**, **rpart**           | tree-based regression
#' 
...
#' 
#' Besides its *class*---which kind of vector it is---a vector's only other necessary attribute is its length.
#' Optionally, vectors in **R** can have a `names` attribute, which allows you to refer to entries by name.
#' 
...
#' By default, **R** displays 7 significant digits.
...
#' 
#' As was mentioned above, vectors can have names associated with their elements:
...
#' Indexing is a little different too: use `[[ ]]` to extract an element of a list by number or name or `$` to extract an element by name (only).
...
#' 
#' Some of the probability distributions built in to **R**.
...
#' Multinomial | `multinom` | size, $\mathrm{prob}\in[0,1]^m$ | $\{(x_1,\dots,x_m): \sum\!x_i=\mathrm{size}\}$
...
#' 
#' Modeling and complicated data analysis are often accomplished more efficiently using *scripts*, which are a series of  commands stored in a text file. 
...
#' you **should not** use MS Word or other fancy editors (see below).
#' 
...
#' 
#' <!--
#' %% **Important:** before working with an example file, create a personal copy in some location on your own computer. 
#' %% We will refer to this location as your *temp folder*. 
#' %% At the end of a lab session you **must** move files onto your personal disk (or email them to yourself).
#' -->
#' 
## ------------------------------------------------------------------------
...
#' Think of these as shorthand for "`Light` = everything in column 1 of `X`", and "`rmax` = everything in column 2 of `X`" (we'll learn about working with data matrices later). From there on out it's the same as before, with some additions
...
#' This and many other **R** gotchas are described in the useful [*R Inferno* [@Burns2012]](http://www.burns-stat.com/pages/Tutor/R_inferno.pdf).
#' 
#' 
#' 
#' ### `while` loops
